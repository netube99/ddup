...
# conditions 声明式规则顶层键（strategy_loader 经本模块 re-export 共享同一份；
# 这里覆盖 cls(config=...) 直接构造路径——EDGE-09）。
...
def validate_conditions(rules: dict) -> dict:
    """conditions 声明式规则顶层键/取值校验（loader 与 ConditionBuilder 共用）。

    与 loader 同口径 fail-fast；返回浅拷贝，调用方改写不落在原 dict 上。
    """
...
class ConditionBuilder:
    ...
    def __init__(self, rules: dict):
        self._rules = validate_conditions(rules or {})
