...
面向 A 股的日频量化研究与回测引擎。因子以声明式 YAML 定义，策略以 Python 子类编写，行情数据库以表单映射接入。引擎负责撮合、费用、滑点、统计及全链路前视屏蔽，研究与回测共享同一因子计算路径。
...
`btcore/` 只提供机制——数据加载、撮合、成本、统计、因子物化。不含任何因子公式或买卖规则。用户代码只声明内容——数据位置、因子表达式、买卖逻辑。两层通过接口契约解耦，引擎升级不影响已有策略。

### 声明式优先

因子公式、条件单、数据位置均在 YAML 或配置 dict 中声明。仅买卖逻辑需以 Python 编写——这是策略的实质，无法声明化。
...
| 配置费率与滑点 | `config` 引擎键 | [策略设计指南](./strategy_guide.md) |
...
在 `strategies/` 下创建 YAML 与 Python 文件。从 `bare_bones` 示例开始，逐级叠加条件单、自管理调仓。详见 [策略设计指南](./strategy_guide.md)。
...
- **[策略设计指南](./strategy_guide.md)** — YAML 完整参考、Python 接口、条件单、逐级教程
- **[CLI 与研究工具](./cli_and_research.md)** — 回测运行、报告生成、对比分析、Brinson 归因、合成因子
- **[ML 子系统指南](./ml_guide.md)** — 面板/持仓双轨模型、训练部署（同源物化）、ONNX 推理、可观测性
