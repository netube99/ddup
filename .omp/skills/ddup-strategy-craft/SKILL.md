...
5. **边界**：空名单合法；buy 名单路径同向信号不加仓（引擎默认；target_value 模式会加仓并按加权均价更新成本、整仓锁一日）；反向信号不对锁（要写进 calc_conditions）；涨跌停/量不足=静默跳过并 warn
...
| L2 | target_value 精确仓位、半仓保留、时间门控降频 | target_allocator |
...
每日时序（有 bar 的交易日）：公司行为调整 → 撮合昨日信号 → NAV 结算 → `on_fills` → `on_tick` → `select` → `calc_conditions` → 生成明日 pending。**无 bar 日整日跳过、昨日 pending 保留到下一交易日执行；"select 每日运行"仅指交易日，非日历日。**
...
- 类变量：`REQUIRED_FIELDS` 默认 ["open","high","low","close","vol","adj_factor"]——**命令式访问的扩展列必须声明**（未声明被列裁剪丢弃，决策时点 KeyError；基础 OHLCV/因子列自动覆盖无需声明）；`FACTOR_SPECS` 条目 {factor*, weight=1.0, ascending=False, materialize_only=False}（ml_ 列引用规则见 ddup-ml-research；materialize_only=只物化不参与评分）；`FILTER_RULES`={}；`CONDITION_FACTORS`=set()（calc_conditions/条件单 handler 读取的因子名声明；loader 交叉校验：与评分因子重叠或未登记 → 警告）
...
- 空返回 {"buy": [], "sell": []} 合法（示例惯例）；空 bars 时引擎不调 select、pending 保持上一日
- **条件买单最后执行**：同日 sell 释放的现金可供 buy_conditions 使用（引擎撮合顺序：手动卖 → 手动买 → 条件单）
...
- 涨跌停：涨停不买/跌停不卖（跌停卖单告警 break，条件单次日由 calc_conditions 重算——**策略须继续声明才离场，无引擎级顺延**）；量 cap、100 股整手、现金不足跳过不缩股
- target_value target=0 清仓：零碎股一并卖出、**不受 100 股整手与成交量 cap 约束**（与普通卖出不同）
- 红利税预扣：现金分红按除息日持股时长预扣——≤30 天 20%、≤365 天 10%、>365 天免征（btcore/corporate.py；预扣方向保守，真实规则卖出时按最终持有期补扣/退还）
- 除权除息日 trailing 锚点（持仓期最高收盘价）按除权比例同步缩放（corporate.adjust 后同步 ConditionBuilder），避免价格台阶被误判为回撤而误触发移动止盈
- 无当日行情（停牌/缺数据）：当日该标的全部订单跳过并告警；卖出不顺延——pending 每日由 select() 重算，平仓意图需次日重新声明。除息日停牌：无 bar 无法按 pre_close rescale 锚点，复牌后存在除权差（已知取舍）
