...
展示能力：
  ...
  - 卖出条件：排名 > top_k × sell_rank_mult 且持有 ≥ min_hold_days（交易日）
  - 买入条件：有空位就从排名前列补入
  - 持有天数读引擎快照 holding_days（交易日口径），不做日期整数差
...
class SelfManagedRank(Strategy):
    ...
    def select(self, bars, account_snapshot, provider) -> dict:
        ...
        for sym in current:
            ...
            # 引擎 holding_days 为交易日口径，成交当日 = 1 → 0-based 持有时长
            held_days = account_snapshot.holdings[sym].holding_days - 1
